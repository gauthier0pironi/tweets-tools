#
# Connects to twitter stream API and dumps tweets in a db
#
...
number_of_tweets_in_mysql_insert_batch=20
keyword = 'computer'
#####
...
insert_query = "CREATE TABLE streaming.`tweets_" + keyword +"` (" + """
...
try:
    ...
    print 'Table tweets_' + keyword + ' created.'
except Exception as e:
    print "error",e

print "Listening to stream for keyword :" +"'" + keyword + "'"

tweet_treated = 0
tweet_to_insert = 0 
query = 'INSERT INTO streaming.tweets_'+ keyword + '  VALUES'
twitter_sample_stream = TwitterStream(auth=auth, domain='stream.twitter.com')
for msg in twitter_sample_stream.statuses.filter(track = keyword, language = 'en'):
#for msg in twitter_sample_stream.statuses.sample(language = 'en'): 
    try:
        if 'text' in msg:
            ...
            if tweet_to_insert == number_of_tweets_in_mysql_insert_batch:
                query += '("' + str(id) + '","'+ time_created+ '",NOW(),"' + tweet.replace('"','\\"') + '");'
                db.query(query)
